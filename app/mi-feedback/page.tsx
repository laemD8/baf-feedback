...
export default function MyFeedbackPage() {
  ...
  async function verify() {
    ...
    const { data } = await supabase.from('feedbacks').select('*')
      .eq('para_persona_id', personaId).eq('sesion_id', sesionId).order('created_at', { ascending: true })
    setFeedbacks(data || [])
  ...
  return (
    <Shell>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-1)', letterSpacing: '-0.01em' }}>My feedback</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {sesionActiva && (
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '16px 20px' }}>
              <p style={{ fontSize: '9px', letterSpacing: '0.1em', color: 'var(--text-3)', textTransform: 'uppercase', marginBottom: '6px' }}>Session in progress</p>
          ...
          <Card>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              <div>
                ...
                <Select value={personaId} onChange={setPersonaId} placeholder="Select your name..." options={personaOptions} />
              </div>
              {sesiones.length > 0 ? (
                <>
                  <div>
                    <FieldLabel>Which session?</FieldLabel>
                    <Select value={sesionId} onChange={setSesionId} placeholder="Select a session..." options={sessionOptions} />
                  </div>
      ...
      {step === 'pin' && (
        <Card>
          ...
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-1)', marginBottom: '6px' }}>Enter your PIN</h2>
          <p style={{ fontSize: '12px', color: 'var(--text-2)', marginBottom: '28px' }}>4-digit code assigned by your admin.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <PinInput value={pin} onChange={setPin} />
            {error && <p style={{ fontSize: '11px', color: 'var(--pink)', textAlign: 'center' }}>{error}</p>}
            ...
            <button onClick={() => { setStep('select'); setPin(''); setError('') }}
              style={{ background: 'none', border: 'none', color: 'var(--text-3)', fontSize: '11px', cursor: 'pointer', fontFamily: 'var(--mono)', padding: '6px' }}>
              ← Back
            </button>
          </div>
        </Card>
      ...
      {step === 'result' && (
        <Card>
          ...
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-1)', marginBottom: '4px' }}>{persona?.nombre}</h2>
          ...
          {feedbacks.length > 0 ? feedbacks.map((fb, i) => (
            <div key={fb.id} style={{
              ...
              <p style={{ fontSize: '9px', letterSpacing: '0.1em', color: 'var(--text-3)', textTransform: 'uppercase', marginBottom: '10px' }}>
                #{String(i + 1).padStart(2, '0')}
              </p>
              <div style={{ fontSize: '13px', color: 'var(--text-2)', lineHeight: 1.8 }}>
                <ReactMarkdown components={{
                  strong: ({ children }) => <span style={{ color: 'var(--text-1)', fontWeight: 700 }}>{children}</span>,
            ...
            <p style={{ fontSize: '13px', color: 'var(--text-3)' }}>No feedback received this session.</p>
          )}
          <Divider />
          <button onClick={() => { setStep('select'); setPin(''); setFeedbacks([]) }}
            style={{ background: 'none', border: 'none', color: 'var(--text-3)', fontSize: '11px', cursor: 'pointer', fontFamily: 'var(--mono)', padding: 0 }}>
